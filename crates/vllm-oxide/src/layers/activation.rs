//! `SiluAndMul` activation — SwiGLU's element-wise half (nano-vllm
//! `activation.py`).
//!
...
//!
//! Lives in `Qwen3Mlp` as the activation between the fused gate-up projection
...
mod tests {
    ...
    mod shape {
        ...
        fn output_last_dim_is_half_input_last_dim() {
            ...
            let out = SiluAndMul::new().forward(&x).unwrap();
