...
pub type ModelFactory = fn(config_json: &[u8], source: Source, device: &Device, max_model_len: usize) -> Result<BuiltModel>;
        ...
        Source::Hub { repo, revision } => {
            ...
            if crate::config::is_hf_hub_offline() {
                ...
                let rh = cache.repo(hf_hub::Repo::with_revision(
                    repo.clone(), hf_hub::RepoType::Model, rev.to_string()));
                rh.get("config.json").ok_or_else(|| {
